...
- **同时上电时，下位机的 boot `READY` 结构上捕获不到**（2026-09-04 实测定案）。
  下位机在 `setup()` 第一句就发 `READY`（约几十毫秒），而本板装好 UART 要 400–500 ms，
  差一个数量级。因此冷启动的判据**不是** `READY`，而是窗口到期后主动问一次 `STATUS`：

  | STATUS 回报 | 处置 |
  |---|---|
  | `ST=IDLE` 且 `ATT=3F` | 置 `position_known` —— 下位机只有归位完成、未被中断、未 ESTOP 才会六轴全 attach 且 IDLE，**那时逻辑角就是物理角** |
  | `RELAXED` / `ESTOP` / `ATT≠3F` / 无回音 | 落锁，由 `self.arm.home` 显式恢复 |

  仍是 fail-closed，只是把证据从抓不到的 `READY` 换成了等价且抓得到的状态。
  **不据此强制 `home`** 是有意的：冷启动时臂可能停在任意姿态，那条 HOME 就是一次大偏置
  开环阶跃，正是 2026-09-01 欠压腐蚀事故的触发候选。
  **运行中**的对端复位不受影响——那时我们在监听，`READY` 抓得到，`link.epoch` 会自增。
