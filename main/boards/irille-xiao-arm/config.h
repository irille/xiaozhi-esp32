...
// ------------------------------------------------------------------ 相机异步分析
// 图像分析**没有时间上限**：大模型思考 30–60 s 属正常，结果何时到就何时能取，
// explain_result 在此期间一律回 pending，**绝不按时间作废结果**。
//
// 本常数只管一件事：新的 take_photo 到来、而旧任务仍在跑时怎么办——
//   旧任务年龄 <  本值 ⇒ 回「相机忙」
//   旧任务年龄 ≥ 本值 ⇒ 判定旧任务陈旧，其结果作废（job id 不匹配即丢）
// 它是给「服务端失联、worker 卡在无超时的 socket 读上」兜底的，不是给正常推理设限。
#ifndef CAMERA_EXPLAIN_STALE_S
#define CAMERA_EXPLAIN_STALE_S 90
#endif
